...
import { createStrategy, getStrategies } from '../services/strategyAPI';
...
export default function StrategyBacktestPage() {
  ...
  const strategyHint = useMemo(() => {
    if (strategyForm.strategy_type === 'rsi') {
      return 'RSI 低于买入阈值开仓，高于卖出阈值平仓。';
    }
    if (strategyForm.strategy_type === 'momentum') {
      return '动量高于阈值开仓，低于负阈值平仓。';
    }
    return '短均线上穿长均线开仓，下穿平仓。';
  }, [strategyForm.strategy_type]);

  const buildStrategyParameters = () => {
    const strategyType = strategyForm.strategy_type;
    if (strategyType === 'rsi') {
      return {
        rsi_period: Math.max(2, Math.floor(toNumber(strategyForm.rsi_period))),
        rsi_buy: toNumber(strategyForm.rsi_buy),
        rsi_sell: toNumber(strategyForm.rsi_sell),
      };
    }
    if (strategyType === 'momentum') {
      return {
        momentum_period: Math.max(2, Math.floor(toNumber(strategyForm.momentum_period))),
        momentum_threshold: toNumber(strategyForm.momentum_threshold),
      };
    }
    return {
      short_window: Math.max(2, Math.floor(toNumber(strategyForm.short_window))),
      long_window: Math.max(3, Math.floor(toNumber(strategyForm.long_window))),
    };
  };
  ...
  const handleRunBacktest = async (event) => {
    ...
    try {
      ...
      setNotice({ type: 'error', message: getErrorMessage(error, '执行回测失败') });
    } finally {
              ...
              <tbody className="divide-y divide-gray-100">
                {backtests.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-sm text-gray-700">{(item.symbols || []).join(', ')}</td>
                    <td className={`px-3 py-2 text-sm text-right ${item.total_return >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                      {fmtPercent(item.total_return)}
                    </td>
                    <td className="px-3 py-2 text-sm text-right text-gray-700">{item.trade_count}</td>
