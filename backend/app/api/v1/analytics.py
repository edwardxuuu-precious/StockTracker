...
def _csv_for_summary(summary: PortfolioSummaryResponse) -> str:
    buffer = StringIO()
...
def _csv_for_holdings(holdings: list[AllocationItemResponse]) -> str:
    buffer = StringIO()
    writer = csv.writer(buffer)
    writer.writerow(
        [
...
async def export_portfolio_analytics_csv(
    ...
    if report == "summary":
        csv_content = _csv_for_summary(summary)
    elif report == "holdings":
        csv_content = _csv_for_holdings(_build_allocation(holdings))
    else:
